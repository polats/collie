...
export function loadConfig(): Config {
  ...
  return {
    ...
    authToken: (process.env.COLLIE_AUTH_TOKEN ?? "").trim(),
    githubOwner: (process.env.COLLIE_GITHUB_OWNER ?? "").trim(),
    checkoutCommand: (process.env.COLLIE_CHECKOUT_COMMAND ?? "").trim(),
    checkoutCwd: (process.env.COLLIE_CHECKOUT_CWD ?? "").trim() || homedir(),
    checkoutTokenFile: (process.env.COLLIE_CHECKOUT_TOKEN_FILE ?? "").trim(),
